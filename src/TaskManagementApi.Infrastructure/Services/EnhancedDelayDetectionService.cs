...
namespace TaskManagementApi.Infrastructure.Services
{
    public class EnhancedDelayDetectionService : BackgroundService
    {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    foreach (var ticket in activeTickets)
                    {
                        ...
                        if (ticket.Status?.Name == "Paused")
                        {
                            ...
                            if (lastStatusChange != null && (now - lastStatusChange.ChangedAt).TotalDays > 4) // Simplistic 3 business days
                            {
                        ...
                        var isBlocked = ticket.IncomingLinks.Any(l => l.LinkType == TicketLinkType.Blocks);
                        if (isBlocked)
                        {
                            // Logic to check how long it's been blocked
        ...
        private async Task HandleDelayAsync(Ticket ticket, DelayType type, DateTime now, IRepository<DelayRecord> repo, IRepository<EscalationRule> escRepo, INotificationService notif)
        {
            ...
            // Skip escalation if RevisedDueDate is set
            if (existing != null && existing.RevisedDueDate.HasValue) return;
            ...
            if (existing == null)
            {
                ...
                await notif.SendAsync(new NotificationEvent { Type = NotificationEventType.TicketOverdue, ProjectId = ticket.ProjectId, ReferenceId = ticket.Id, Title = "Delay Detected" });
            }
            else
            {
                ...
                var rule = await escRepo.Query().FirstOrDefaultAsync(r => r.ProjectId == ticket.ProjectId) ?? new EscalationRule { EscalateAfterDays = 3, SecondLevelAfterDays = 7 };

                if (days >= rule.SecondLevelAfterDays && existing.EscalationLevel < 2)
                {
                    existing.EscalationLevel = 2;
                    await notif.SendAsync(new NotificationEvent { Type = NotificationEventType.TicketOverdue, ProjectId = ticket.ProjectId, ReferenceId = ticket.Id, Title = "Director Escalation" });
                }
